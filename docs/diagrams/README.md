...
| `ehds-operating-model-wide.svg`        | The **slide overview** of the same model: four bands, chips instead of boxes, one article line per band. Built to be read while somebody is talking over it. The file above is the reference version and keeps the detail.                                                                                       |
