...
**Option C.** The connectors are core to what the project demonstrates — deleting all of them turns the Azure demo into a clinical-graph viewer with mock dataspace overlays, which contradicts the README. CFM + OMOP-dataplane + NATS are auxiliary and can come back later if needed.
